...
const App = () => {
  ...
  const fetchFacultyData = async () => {
    ...
    try {
      const response = await fetch(`https://back-w3b4.onrender.com/faculty-profile/${facultyId}`);
      if (!response.ok) {
  ...
  const handleBooking = async () => {
    ...
    try {
      const response = await fetch("https://back-w3b4.onrender.com/book-room", {
        method: "POST",
  ...
  const handleProfileUpdate = async () => {
    ...
    try {
      const response = await fetch(`https://back-w3b4.onrender.com/update-profile`, {
        method: "PUT",
        headers: {
          "Authorization": `Bearer ${localStorage.getItem("token")}`,
        },
