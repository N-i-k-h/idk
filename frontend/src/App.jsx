...
const App = () => {
  ...
  const fetchFacultyData = async () => {
    ...
    try {
      const response = await fetch(`https://idk-5ba.onrender.com/faculty-profile/${facultyId}`);
      if (!response.ok) {
  ...
  const handleBooking = async () => {
    ...
    try {
      const response = await fetch("https://idk-5ba.onrender.com/book-room", {
        method: "POST",
  ...
  const handleProfileUpdate = async () => {
    ...
    try {
      const response = await fetch(`https://idk-5ba.onrender.com/update-profile`, {
        method: "PUT",
        headers: {
          "Authorization": `Bearer ${localStorage.getItem("token")}`,
        },
