...
const Admin = () => {
  ...
  useEffect(() => {
    const fetchDateHistory = async () => {
      try {
        const response = await fetch("http://localhost:5000/admin/date-history", {
          method: "GET",
          headers: {
  ...
  useEffect(() => {
    const fetchFacultyList = async () => {
      if (selectedBranch) {
        try {
          const response = await fetch(`http://localhost:5000/admin/faculty-list?branch=${selectedBranch}&page=${currentPage}`, {
            method: "GET",
            headers: {
              "Content-Type": "application/json",
              Authorization: `Bearer ${localStorage.getItem("adminToken")}`,
  ...
  const handleConfirm = async () => {
    try {
      const response = await fetch("http://localhost:5000/admin/add-date", {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
          Authorization: `Bearer ${localStorage.getItem("adminToken")}`,
  ...
  const handleResetDates = async () => {
    try {
      const response = await fetch("http://localhost:5000/admin/reset-dates", {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
          Authorization: `Bearer ${localStorage.getItem("adminToken")}`,
